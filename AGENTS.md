...
### 地は白ではなく slate-50 で測る

`globals.css` の `--background` は白磁 `#f8f6f1` で、`html, body` に当たっている。
ただし `MobileAppShell` のルート div が `min-h-[100dvh]` の `bg-slate-50`
（`#f8fafc`）で全面を覆うので、**文字が実際に載る地は slate-50**。
白（`#fff`）で測ると 0.2 前後ずれて、境界のクラスの合否が入れ替わる。

| 文字色 | 白 `#fff` | slate-50（実際の地） | slate-100 | 金の塗り brand-700 |
|---|---|---|---|---|
| slate-400 | 2.56 ✗ | 2.45 ✗ | 2.34 ✗ | — |
| slate-500 | 4.76 ✓ | 4.55 ✓ | **4.34 ✗** | — |
| slate-600 | 7.58 ✓ | 7.24 ✓ | 6.92 ✓ | — |
| brand-600 | 3.79 ✗ | 3.63 ✗ | 3.46 ✗ | — |
| brand-700 | 5.34 ✓ | 5.11 ✓ | 4.88 ✓ | — |
| 白文字 | — | — | — | brand-600 = 3.79 ✗ / brand-700 = 5.34 ✓ |
