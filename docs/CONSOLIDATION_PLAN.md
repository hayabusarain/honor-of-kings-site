...
静的書き出し（サーバーを持たない形）にそろえる方針は、ワイリフだけ例外が要る。カウンター投票の保存がサーバー無しでは動かないため（4章）。
...
作業量の目安は、試作1日、ポータル1日、HoK 1.5日、MLBB 半日〜1日、Wild Rift 2〜3日（サーバー機能の置き換え）、切り替え半日。

## 6. 決めてほしいこと

1. **Cloudflare のプラン**: 無料で足りるのは、スクリプトを動かさない場合。投票を Worker で残すか、言語判定をスクリプトでするなら、無料枠（1日10万回）を使う
2. **Wild Rift のカウンター投票**: Worker で残すか、やめるか
3. **ポータルの適性診断**: 結果の表示に JavaScript が要る形にしてよいか
4. **言語の自動振り分け**: `/hok` を開いた人を、ブラウザの言語で振り分け続けるか（小さなスクリプトが要る）、`/hok/ja` へ固定で送るか。いまは HoK の既定が英語、MLBB は日本語固定で、サイトごとに違う
5. **アクセス解析**: 1つの測定 ID のままパスで分けるか、分けるか
6. **確かめてほしいこと**（運営者の画面でしか見られない）: Search Console に hub-game.com のドメインプロパティがあるか。AdSense の再審査を切り替えの前と後のどちらで出すか
