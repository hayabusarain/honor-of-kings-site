import cv2
import json
import numpy as np
import os

def read_img_unicode(path):
    return cv2.imdecode(np.fromfile(path, dtype=np.uint8), cv2.IMREAD_COLOR)

folder = r'C:\Users\81901\Desktop\おなきんあいてむ'
p_items = os.path.abspath(r'src\data\hok_items.json')

with open(p_items, 'r', encoding='utf-8') as f:
    items = json.load(f)

files = [f for f in os.listdir(folder) if f.endswith('.png')]
print(f"Batch processing all {len(files)} screenshot files in {folder}...")

# Mask to crop circular item icons
mask = np.zeros((64, 64), dtype=np.uint8)
cv2.circle(mask, (32, 32), 27, (255, 255, 255), -1)

updated_list = []

for filename in files:
    filepath = os.path.join(folder, filename)
    img = read_img_unicode(filepath)
    if img is None:
        continue
        
    # Crop right synthesis top icon (y: 285 to 375, x: 1205 to 1295)
    icon_crop = img[285:375, 1205:1295]
    if icon_crop is None or icon_crop.shape[0] < 50 or icon_crop.shape[1] < 50:
        continue
        
    icon_sq = cv2.resize(icon_crop, (64, 64))
    icon_masked = cv2.bitwise_and(icon_sq, icon_sq, mask=mask)
    
    best_score = -1
    best_item = None
    
    for it in items:
        icon_path = os.path.join('public', it['icon'].lstrip('/'))
        if os.path.exists(icon_path):
            ref_img = read_img_unicode(icon_path)
            if ref_img is not None:
                ref_sq = cv2.resize(ref_img, (64, 64))
                ref_masked = cv2.bitwise_and(ref_sq, ref_sq, mask=mask)
                res = cv2.matchTemplate(icon_masked, ref_masked, cv2.TM_CCOEFF_NORMED)
                _, max_val, _, _ = cv2.minMaxLoc(res)
                if max_val > best_score:
                    best_score = max_val
                    best_item = it
                    
    if best_item:
        site_icon_rel = best_item['icon'].lstrip('/')
        save_path = os.path.join('public', site_icon_rel)
        
        # Save 160x160 crisp crop
        highres_crop = cv2.resize(icon_crop, (160, 160))
        cv2.imwrite(save_path, highres_crop)
        
        updated_list.append({
            "screenshot": filename,
            "id": best_item['id'],
            "name": best_item['name'],
            "icon_path": site_icon_rel,
            "match_score": round(float(best_score), 3)
        })

print(f"\nBatch completed! Updated {len(updated_list)} item icons across the site!")

with open('scratch/all_110_items_cropped_report.json', 'w', encoding='utf-8') as f:
    json.dump(updated_list, f, ensure_ascii=False, indent=2)
