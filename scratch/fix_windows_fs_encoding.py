import ctypes
import json
import os
import re

MoveFileW = ctypes.windll.kernel32.MoveFileW

base_folder = r'C:\Users\81901\Pictures\Screenshots'

with open('scratch/all_hero_folder_ocr_utf8.json', 'r', encoding='utf-8') as f:
    ocr_data = json.load(f)

with open('src/data/hok_heroes.json', 'r', encoding='utf-8') as f:
    heroes = json.load(f)

current_subdirs = sorted([d for d in os.listdir(base_folder) if os.path.isdir(os.path.join(base_folder, d)) and d.startswith("Set_")])

print(f"Fixing Windows FS encoding for {len(current_subdirs)} set folders using MoveFileW...")

renamed_count = 0

for i, cd in enumerate(current_subdirs):
    m = re.search(r'Set_(\d+)_(?:.*_)?(\d+-\d+)', cd)
    if not m:
        continue
    set_num = m.group(1)
    range_str = m.group(2)
    
    ocr_text = ocr_data[i]['text'] if i < len(ocr_data) else ""
    
    matched_hero = None
    if ocr_text:
        # Match exact hero name in hok_heroes.json
        for h in heroes:
            hname = h['name']
            hname_en = h.get('name_en', '')
            if hname in ocr_text or (hname_en and len(hname_en) >= 3 and hname_en.lower() in ocr_text.lower()):
                matched_hero = hname
                break
                
        if not matched_hero:
            for h in heroes:
                hname = h['name']
                if len(hname) >= 2 and hname[:2] in ocr_text:
                    matched_hero = hname
                    break

    hero_name = matched_hero if matched_hero else "UNKNOWN"
    new_dir_name = f"Set_{set_num}_{hero_name}_{range_str}"
    
    old_path = os.path.join(base_folder, cd)
    new_path = os.path.join(base_folder, new_dir_name)
    
    if old_path != new_path:
        res = MoveFileW(old_path, new_path)
        if res != 0:
            renamed_count += 1
            print(f"  [{set_num}] SUCCESS: Set {set_num} -> '{new_dir_name}'")
        else:
            err = ctypes.GetLastError()
            print(f"  [{set_num}] MoveFileW Error {err}: '{cd}' -> '{new_dir_name}'")

print(f"\nCOMPLETED! Cleanly renamed {renamed_count} folders on Windows File System!")
