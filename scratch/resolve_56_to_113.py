...
print(f"Resolving remaining UNKNOWN set folders across all 11 screenshot images (056 to 113)...")
...
for d in subdirs:
    ...
    try:
        proc = subprocess.run([r'C:\Users\81901\Desktop\オナーオブキングスサイト\scratch\ocr_folder.exe', os.path.abspath(sd_path)], capture_output=True, text=True, encoding='utf-8', errors='ignore')
        full_txt = proc.stdout.replace(" ", "").replace("\n", "").replace("\r", "")
    except Exception as ex:
        print(f"Error running OCR on {d}: {ex}")
        full_txt = ""
    ...
    if matched_hero:
        new_dir = f"Set_{set_num:03d}_{matched_hero}_{range_str}"
        ...
        if old_p != new_p:
            try:
                ...
                renamed_count += 1
                print(f"  [{set_num:03d}] RESOLVED: '{d}' -> '{new_dir}'")
            except Exception as e:
                print(f"  [{set_num:03d}] Error: {e}")
