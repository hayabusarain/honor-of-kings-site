 ...
 *   代わりに、未来形で書いている5箇所を過去形へ振り直す。
 *   このファイルの ja.lead / ja.facts / ja.notConfirmed と、en の対応箇所。
 *   「まだ分かっていないこと／What is not confirmed yet」の節は、結果が出たあとは
 *   残さず、確認できた出場国と結果に差し替えるか、節ごと落とす。
...
export const ASIAN_GAMES_2026 = {
  ...
  /**
   * トップページのバナーを出す期限。競技日は9月28日の1日だけなので、翌日0時に落とす。
   * Vercel は UTC で動くので、オフセット（+09:00）を省くと最大9時間ずれる。
   * eスポーツ競技期間の終わり（10月2日）まで出したいなら
   * '2026-10-03T00:00:00+09:00' に書き換える。ただしバナー本文が「9月28日」と
   * 名指ししているので、その場合は本文も併せて直すこと。
   */
