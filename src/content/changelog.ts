...
export const CHANGELOG: ChangelogEntry[] = [
  {
    date: '2026-09-26',
    ja: 'サイト全体を墨の地に金の配色へ作り直し、文字はすべて14px以上にした。ロールごとのヒーロー一覧（6ロール）を追加し、Tier・レーン・難易度の内訳を載せた。用語集を独立したページにした。Tier表とヒーロー詳細では、前回（9月4日取得）の統計からのTierと勝率の変化が分かる。ヒーロー詳細から2体比較のページへ移れるようにした。新ヒーロー「元流の子（アサシン）」「元流の子（サポート）」の立ち回り解説を書き直した。アジア競技大会のページに、出場する10の国と地域・組み分け・試合形式・日本からの観かたを足し、日程を9月27〜28日の2日間に直した。',
    en: 'Redesigned the whole site in a dark ink-and-gold theme, with no text smaller than 14px. Added a hero list for each of the six roles, with its tier, lane and difficulty breakdown. The glossary now has its own page. The tier list and hero pages show how each hero\'s tier and win rate moved since the previous stats (taken September 4). Hero pages now link to the two-hero comparison. Rewrote the strategy sections for the new heroes Flowborn (Assassin) and Flowborn (Roamer). The Asian Games page now lists the ten competing nations and regions, the groups, the format and how to watch from Japan, and the schedule is corrected to two days, 27–28 September.',
