...
import { ChevronRight, Crosshair, HeartPulse, MapIcon, Shield, SlidersHorizontal, Sparkles, Target, Zap, type LucideIcon } from 'lucide-react';
...
const ROLE_ICON: Record<RoleId, LucideIcon> = {
  Fighter: Target,
  Tank: Shield,
  Mage: Sparkles,
  Assassin: Zap,
  ...
  return (
    <li>
      <Link href={hero.href} className="group flex h-full flex-col items-center gap-1.5">
        <span className="relative h-[76px] w-[76px] overflow-hidden rounded-2xl border border-slate-200 bg-slate-100 shadow-sm sm:h-20 sm:w-20">
          ...
          {hero.tier && (
            <span aria-hidden="true" className={`absolute top-0 right-0 rounded-bl-lg border px-1.5 py-0.5 text-xs font-black shadow-xs ${getTierBadgeStyle(hero.tier)}`}>
              {hero.tier}
            </span>
          )}
          ...
          {hero.prePatch && (
            <span aria-hidden="true" className="absolute inset-x-0 bottom-0 border-t border-amber-200 bg-amber-50 text-center text-xs font-bold leading-tight text-amber-800">
        ...
        <span className="flex w-full flex-1 flex-col items-center gap-0.5">
          ...
          <span className="flex w-full flex-col items-center text-sm font-bold leading-tight text-slate-800 transition-colors group-hover:text-brand-700">
            <span className={`w-full text-center ${loc === 'en' ? 'line-clamp-2 break-words' : 'truncate'}`}>
              {nameBase}
            </span>
            {nameQualifier && <span className="w-full truncate text-center text-xs">{nameQualifier}</span>}
          </span>
          ...
          <span className="mt-auto flex flex-col items-center">
            {hero.lane && (
              <span className="text-xs font-bold leading-tight text-slate-500">{laneShort[hero.lane]}</span>
            )}
            {hero.winRate !== null ? (
              ...
              <span className="text-xs font-bold leading-tight text-slate-500">{ROLE_UI.noStats[loc]}</span>
            )}
          </span>
        </span>
      </Link>
...
export function RoleLandingView({ locale, data, laneShort, otherRoles }: Props) {
  ...
  const RoleIcon = ROLE_ICON[landing.id];
  ...
  return (
    ...
    <div className="w-full bg-background px-1 pb-10">
      <div className="flex items-center gap-2.5 pt-2">
        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-brand-700/40 bg-white">
          <RoleIcon size={20} className="text-brand-700" aria-hidden="true" />
        </span>
      ...
      <section aria-labelledby="role-summary" className="mt-4 rounded-2xl border border-slate-200 bg-white p-4 sm:p-5">
        <h2 id="role-summary" className="text-base font-black text-slate-800">
        ...
        {caveats.length > 0 && (
          <div className="mt-3 max-w-3xl space-y-1">
            {caveats.map((c, i) => (
              <p key={i} className="text-xs font-bold leading-relaxed text-slate-600">{c}</p>
            ))}
          </div>
        )}
      ...
      <section aria-labelledby="role-grid" className="mt-8">
        <h2 id="role-grid" className="text-base font-black text-slate-800">
          {ROLE_UI.gridHeading[loc](roleName)}
        </h2>
        <p className="mt-1 text-xs font-medium leading-relaxed text-slate-500">{ROLE_UI.gridOrder[loc]}</p>
      ...
      <section aria-labelledby="role-more" className="mt-10">
        <h2 id="role-more" className="text-base font-black text-slate-800">{ROLE_UI.moreHeading[loc]}</h2>
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            <span className="block text-sm font-black text-slate-900 group-hover:text-brand-700">{ROLE_UI.toList[loc](roleName)}</span>
            <span className="mt-0.5 block text-xs font-medium leading-relaxed text-slate-600">{ROLE_UI.toListDesc[loc]}</span>
          </span>
              ...
              <li key={r.slug}>
                ...
                >
                  ...
                  <span className="flex min-w-0 flex-col leading-tight">
                    ...
                    <span className="text-xs font-bold tabular-nums text-slate-500">{ROLE_UI.count[loc](r.count)}</span>
                  </span>
                </Link>
              </li>
            );
