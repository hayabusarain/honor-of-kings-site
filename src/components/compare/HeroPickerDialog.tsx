...
export function HeroPickerDialog({ locale, heroes, slot, current, taken, onPick, onClear, onClose }: Props) {
  ...
  return (
    <div
      className="fixed inset-0 z-[80] flex items-end justify-center bg-slate-950/50 md:items-center md:p-6"
      onClick={onClose}
    >
      <div
        ref={panelRef}
        ...
        className="flex h-[85dvh] w-full flex-col rounded-t-3xl bg-white pb-[env(safe-area-inset-bottom)] shadow-2xl outline-none md:h-auto md:max-h-[80vh] md:max-w-2xl md:rounded-3xl md:pb-0"
      >
        ...
        <div className="flex gap-2 px-4 pt-1">
          <div className="relative min-w-0 flex-1">
            ...
            <input
              ...
              className="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 pr-3 pl-9 text-base font-bold text-slate-800 outline-none placeholder:text-slate-500 focus:border-slate-400 md:text-sm"
            />
          </div>
          <Dropdown
            label={ja ? 'ロール' : 'Role'}
            className="w-36 shrink-0"
            icon={<LayoutGrid className="h-5 w-5 text-slate-500" aria-hidden="true" />}
            options={[
        ...
        <p aria-live="polite" className="min-h-5 px-4 pt-2 text-xs font-bold text-slate-600">
                  ...
                  <li key={hero.slug}>
                    <button
                      ...
                      className={`flex w-full flex-col items-center gap-1 rounded-xl px-1 py-1.5 transition-colors ${
                        isTaken ? 'cursor-not-allowed opacity-40' : 'hover:bg-slate-100'
                      }`}
                    >
                      <Image
                        src={hero.image}
                        ...
                        className={`h-12 w-12 rounded-xl bg-slate-100 object-cover ${
                          isCurrent ? 'ring-2 ring-slate-900 ring-offset-2' : 'ring-1 ring-slate-200'
                        }`}
                      />
                      <span
                        className={`line-clamp-2 w-full text-center text-xs leading-tight break-words ${
